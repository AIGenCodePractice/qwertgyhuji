...
public class PasswordPolicyTests
{
    ...
    public void Setup()
    {
        ...
        _mockSessions.Setup(r => r.GetByToken(It.Is<string>(tok => tok != _session.Token))).Returns((Session?)null);
        ...
        _mockValidator.Setup(v => v.IsValidPassword(It.IsAny<string>())).Returns((string p) =>
        {
            if (string.IsNullOrWhiteSpace(p) || p.Length < 8) return false;
            return p.Any(char.IsUpper) && p.Any(char.IsLower) && p.Any(char.IsDigit) && p.Any(c => "!@#$%^&*()_+-=[]{}|;:,.<>?".Contains(c));
        });
    ...
    {
        _mockUsers = null; _mockSessions = null; _mockHasher = null; _mockAudit = null; _mockValidator = null; _auth = null; _user = null; _session = null;
    }
    ...
    public void ChangePassword_WeakNewPassword_ReturnsFailure()
    {
        ...
        StringAssert.Contains(result.Message.ToLowerInvariant(), "new password does not meet complexity requirements.");
    ...
    public void ChangePassword_IncorrectOldPassword_ReturnsFailure()
    {
        ...
        Assert.IsFalse(result.IsSuccess);
        StringAssert.Contains(result.Message.ToLowerInvariant(), "current password is incorrect.");
        _mockUsers!.Verify(u => u.Update(It.IsAny<User>()), Times.Never);
    }
    ...
    public void ChangePassword_InvalidSession_ReturnsFailure()
    {
        ...
        _mockSessions.Setup(r => r.GetByToken(It.IsAny<string>())).Returns((string token) => token == _session!.Token ? _session : null);
        var auth = new AuthService(_mockUsers!.Object, _mockSessions.Object, _mockHasher!.Object, _mockAudit!.Object, _mockValidator!.Object);
