...
public class AuthService : IAuthService
{
    ...
    private static readonly Dictionary<UserRole, HashSet<string>> _permissions = new()
    {
        [UserRole.Admin] = new HashSet<string>
        {
            ...
            "APPROVE_LOAN", "REJECT_LOAN", "CREATE_USER", "LOCK_USER", "UNLOCK_USER",
            "VIEW_AUDIT_LOG", "GENERATE_REPORT", "VIEW_ALL_ACCOUNTS"
        },
        [UserRole.Manager] = new HashSet<string>
        {
            "CREATE_ACCOUNT", "UPDATE_ACCOUNT", "DEPOSIT", "WITHDRAW", "TRANSFER",
            "APPROVE_LOAN", "REJECT_LOAN", "VIEW_AUDIT_LOG", "GENERATE_REPORT", "VIEW_ALL_ACCOUNTS"
        },
        [UserRole.Teller] = new HashSet<string>
        {
            "DEPOSIT", "WITHDRAW", "TRANSFER", "VIEW_OWN_ACCOUNTS"
        },
    ...
    public OperationResult<Session> Login(string username, string password)
    {
        ...
        if (user.IsLocked)
        {
            if (user.LockoutExpiry.HasValue && DateTime.UtcNow >= user.LockoutExpiry.Value)
            {
                user.IsLocked = false;
    ...
    public OperationResult<bool> ValidateSession(string token)
    {
        ...
        if (!session.IsActive)
            return OperationResult<bool>.Failure("Session is inactive.");
    ...
    public OperationResult<bool> ChangePassword(string token, string currentPassword, string newPassword)
    {
        ...
        if (!_validator.IsValidPassword(newPassword))
            return OperationResult<bool>.Failure("New password does not meet complexity requirements.");

        (string newHash, string newSalt) = _hasher.HashPassword(newPassword);
        foreach (var oldHash in user.PasswordHistory)
        {
            if (oldHash == newHash)
                return OperationResult<bool>.Failure("New password cannot be the same as a recent password.");
        }
    ...
    public OperationResult<User> RegisterUser(string username, string password, UserRole role, string createdBy)
    {
    ...
    /// <summary>
    /// Checks whether an active, unexpired session owner has permission for the requested operation.
    /// </summary>
