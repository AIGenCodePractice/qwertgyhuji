...
public class LoanService : ILoanService
{
    ...
    private const decimal EarlySettlementFee = 0.015m;
    private const decimal ArrearsMonthlyPenalty = 0.02m;

    public LoanService(ILoanRepository loanRepo, IAccountRepository accountRepo, IAuditService audit)
    {
        _loanRepo = loanRepo;
    ...
    public OperationResult<Loan> ApplyForLoan(int accountId, LoanType type, decimal amount,
        int termMonths, decimal annualRate, decimal monthlyIncome, decimal existingDebt, int creditScore)
    {
        ...
        if (monthlyIncome <= 0)
            return OperationResult<Loan>.Failure("Monthly income must be positive.");
        if (existingDebt < 0)
            return OperationResult<Loan>.Failure("Existing debt cannot be negative.");
        if (creditScore < MinCreditScore)
            return OperationResult<Loan>.Failure($"Minimum credit score of {MinCreditScore} required. Applicant score: {creditScore}.");

        decimal monthlyInstalment = CalculatePMT(amount, annualRate, termMonths);
        decimal dti = (existingDebt + monthlyInstalment) / monthlyIncome;
        ...
        _loanRepo.Add(loan);
        _audit.Log("LOAN_APPLICATION", "SYSTEM", $"Loan application {loan.LoanReference} for R{amount:F2}", loan.LoanReference);
    ...
    public OperationResult<Loan> ApproveLoan(string loanReference, string approvedBy)
    {
        ...
        if (loan.Status != LoanStatus.Pending) return OperationResult<Loan>.Failure("Only Pending loans can be approved.");
    ...
    public OperationResult<Loan> RejectLoan(string loanReference, string reason, string rejectedBy)
    {
        var loan = _loanRepo.GetByReference(loanReference);
        if (loan == null) return OperationResult<Loan>.Failure("Loan not found.");
        if (loan.Status != LoanStatus.Pending) return OperationResult<Loan>.Failure("Only Pending loans can be rejected.");
    ...
    public OperationResult<Loan> ProcessRepayment(string loanReference, decimal amount, string processedBy)
    {
        var loan = _loanRepo.GetByReference(loanReference);
        if (loan == null) return OperationResult<Loan>.Failure("Loan not found.");
        ...
        if (amount <= 0) return OperationResult<Loan>.Failure("Repayment amount must be positive.");

        decimal appliedAmount = Math.Min(amount, loan.OutstandingBalance);
        ...
        else if (loan.Status == LoanStatus.Arrears && appliedAmount >= loan.ArrearsAmount)
        {
        ...
        _loanRepo.Update(loan);
        _audit.Log("LOAN_REPAYMENT", processedBy, $"Repayment of R{appliedAmount:F2} on loan {loanReference}", loanReference);
    ...
    public OperationResult<decimal> CalculateSettlementAmount(string loanReference)
    {
        ...
        decimal fee = loan.OutstandingBalance * EarlySettlementFee;
        return OperationResult<decimal>.Success(loan.OutstandingBalance + fee);
    }
    ...
    {
        var loan = _loanRepo.GetByReference(loanReference);
        return loan == null ? OperationResult<Loan>.Failure("Loan not found.") : OperationResult<Loan>.Success(loan);
    }
    ...
    private static List<LoanRepayment> BuildRepaymentSchedule(Loan loan)
    {
        ...
        for (int i = 1; i <= loan.TermMonths; i++)
        {
            ...
            if (i == loan.TermMonths) closing = 0m;
            ...
            schedule.Add(new LoanRepayment
            {
                ...
                ClosingBalance = Math.Max(0m, closing)
            });

            balance = Math.Max(0m, closing);
            if (balance == 0) break;
        }
        return schedule;
    }
