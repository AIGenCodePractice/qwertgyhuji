...
public class TransactionService : ITransactionService
{
    ...
    public TransactionService(IAccountRepository accountRepo, ITransactionRepository txnRepo,
        ...
        string description, string processedBy)
    {
        ...
        if (amount > MaxSingleTransaction)
            return OperationResult<Transaction>.Failure($"Single deposit cannot exceed R{MaxSingleTransaction:F2}.");

        var account = _accountRepo.GetById(accountId);
        if (account == null)
            return OperationResult<Transaction>.Failure("Account not found.");
        if (account.Status != AccountStatus.Active)
            return OperationResult<Transaction>.Failure("Deposits can only be made to Active accounts.");
        ...
        _txnRepo.Add(txn);
        _audit.Log("DEPOSIT", processedBy, $"Deposit of R{amount:F2} to {account.AccountNumber}", txn.TransactionReference);
        ...
        string description, string processedBy)
    {
        ...
        if (amount > MaxSingleTransaction)
            return OperationResult<Transaction>.Failure($"Single withdrawal cannot exceed R{MaxSingleTransaction:F2}.");

        var account = _accountRepo.GetById(accountId);
        if (account == null)
            return OperationResult<Transaction>.Failure("Account not found.");
        if (account.Status != AccountStatus.Active)
            return OperationResult<Transaction>.Failure("Withdrawals can only be made from Active accounts.");
        ...
        var txn = CreateTransaction(accountId, null, TransactionType.Withdrawal,
            amount, balanceBefore, account.Balance, description, processedBy);
        _txnRepo.Add(txn);
        _audit.Log("WITHDRAWAL", processedBy, $"Withdrawal of R{amount:F2} from {account.AccountNumber}", txn.TransactionReference);
        ...
        decimal amount, string description, string processedBy)
    {
        ...
        if (amount > MaxSingleTransaction)
            return OperationResult<Transaction>.Failure($"Single transfer cannot exceed R{MaxSingleTransaction:F2}.");
        ...
        if (toAccount.Status != AccountStatus.Active)
            return OperationResult<Transaction>.Failure("Destination account is not Active.");
        if (fromAccount.Balance < amount)
            return OperationResult<Transaction>.Failure("Insufficient funds in source account.");
        if (fromAccount.DailyWithdrawnToday + amount > fromAccount.DailyWithdrawalLimit)
            return OperationResult<Transaction>.Failure(
                $"Daily withdrawal limit of R{fromAccount.DailyWithdrawalLimit:F2} would be exceeded.");
        ...
        _audit.Log("TRANSFER", processedBy,
            $"Transfer of R{amount:F2} from {fromAccount.AccountNumber} to {toAccount.AccountNumber}", txn.TransactionReference);
        ...
        string reason, string processedBy)
    {
        ...
        if ((DateTime.UtcNow - original.Timestamp).TotalHours > ReversalWindowHours)
            return OperationResult<Transaction>.Failure($"Reversal window of {ReversalWindowHours} hours has expired.");
        ...
        if (original.Type == TransactionType.Deposit || original.Type == TransactionType.Transfer)
        {
            if (account.Balance < original.Amount)
                return OperationResult<Transaction>.Failure("Insufficient funds to reverse this deposit.");
        ...
        _audit.Log("REVERSAL", processedBy,
            $"Transaction {transactionReference} reversed. Reason: {reason}", reversalTxn.TransactionReference);
        ...
        DateTime? from = null, DateTime? to = null)
    {
        ...
        var txns = from.HasValue && to.HasValue
            ? _txnRepo.GetByDateRange(accountId, from.Value, to.Value)
            : _txnRepo.GetByAccountId(accountId);
        return OperationResult<List<Transaction>>.Success(txns);
    }

    private static Transaction CreateTransaction(int accountId, int? targetId,
        TransactionType type, decimal amount, decimal before, decimal after,
        string description, string processedBy) => new()
    {
